...
from erpnext.accounts.doctype.pos_invoice.pos_invoice import get_stock_availability_all
...
@frappe.whitelist()
def get_items_23_11(start, page_length, price_list, item_group, pos_profile, search_term="", packaging="false"):
	...
	sorted_data = sorted(result, key=lambda x: x["posicion"])

	return {'items': sorted_data}

@frappe.whitelist(allow_guest=True)
def get_items(start, page_length, price_list, item_group, pos_profile, search_term="", packaging="false"):
	warehouse, hide_unavailable_items = frappe.db.get_value(
		'POS Profile', pos_profile, ['warehouse', 'hide_unavailable_items'])
	...
	price_join_selection = ", `tabItem Price` price"
	price_join_condition = "AND price.price_list = %(price_list)s AND price.item_code = item.name "
	if packaging == "true":
		items_data = frappe.db.sql("""
		SELECT
			item.name AS item_code,
			...
			price.currency
		FROM
			`tabItem` item {bin_join_selection} {price_join_selection}
		WHERE
			substring(item.item_code,1,6) = 'SEREMB'
			{condition}
			{bin_join_condition}
			{price_join_condition}
		""".format(
			condition=condition,
			bin_join_selection=bin_join_selection,
			bin_join_condition=bin_join_condition,
			price_join_condition=price_join_condition,
			price_join_selection=price_join_selection
		),  {'warehouse': warehouse,'price_list': price_list}, as_dict=1)
	else:
		items_data = frappe.db.sql("""
		SELECT
			item.name AS item_code,
	...
	sorted_data = sorted(items_data, key=lambda x: x["posicion"])

	return {'items': sorted_data}


@frappe.whitelist()
def get_items_old(start, page_length, price_list, item_group, pos_profile, search_term=""):
	warehouse, hide_unavailable_items = frappe.db.get_value(
		'POS Profile', pos_profile, ['warehouse', 'hide_unavailable_items'])
	...
	if items_data:
		...
		for item in items_data:
			if (pos_profile != "MEXICO 2" and frappe.session.user not in ["77050071@shalomcontrol.com","tienda@shalom.com.pe","administrator","48191841@shalomcontrol.com","74451380@shalomcontrol.com"]) and "MERCHANDISING" in item.item_name:
				continue
			item_code = item.item_code
			item_price = item_prices.get(item_code) or {}
			item_stock_qty = get_stock_availability(item_code, warehouse)
...
def get_conditions(search_term):
	...
	if search_term:
		condition = "("
		condition += """item.name = {search_term}""".format(search_term=search_term)
...
@frappe.whitelist()
def get_items_supervision(price_list, item_group, warehouse, search_term="", packaging="false"):
