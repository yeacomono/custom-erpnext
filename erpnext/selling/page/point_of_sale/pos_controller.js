erpnext.PointOfSale.Controller = class {
	...
	create_opening_voucher() {
		...
		const dialog = new frappe.ui.Dialog({
			...
			fields: [
				...
				},
				{
					...
					default: frappe.defaults.get_user_permissions("user")["POS Profile"] == undefined ? "": frappe.defaults.get_user_permissions("user")["POS Profile"][0]["doc"],
		...
		primary_action: async function({ company, pos_profile, balance_details }) {
			let arrayForValidation = await $.ajax({
				type:"GET",
				url:"https://recursoshumanos.shalom.com.pe/api/pos-validation",
				dataType:"JSON"
			})
			let tiendita = await frappe.db.get_list("CIERRE TIENDA",{
				fields:["name","docstatus","saldo","fecha_cierre"],
				filters:[["pos_profile","=",pos_profile]],
				order_by:'fecha_cierre desc',
				limit:"None",
			})
			/*if(arrayForValidation.includes(pos_profile)){
				tiendita = await frappe.db.get_list("CIERRE TIENDA",{
					fields:["name","docstatus","saldo","fecha_cierre"],
					filters:[["pos_profile","=",pos_profile]],
					order_by:'fecha_cierre desc',
					limit:"None",
				...
				tiendita = await frappe.db.get_list("Caja Tiendita",{
					fields:["name","docstatus","saldo","fecha_cierre"],
					filters:[["pos_profile","=",pos_profile]],
					order_by:'fecha_cierre desc',
					limit:"None",
			...
			let valorForValidate = false;
			let msg = ""


			const hoy = moment(new Date()).format("YYYY-MM-DD");
			const esLunes = moment().day() === 1;
			let fechaReferencia;
			if (esLunes) {
				fechaReferencia = moment().subtract(2, 'days').format("YYYY-MM-DD");
			} else {
				fechaReferencia = moment().subtract(1, 'days').format("YYYY-MM-DD");
			}
			let ultima_venta = await frappe.db.get_list("POS Invoice",{
				fields:["name","posting_date"],
				filters:[
					["pos_profile","=",pos_profile],
					["docstatus","!=",2],
					["posting_date","=",fechaReferencia]
				],
				order_by:'posting_date desc',
				limit:"None",
			})

			let existence_cierre_referencia = 0
			tiendita.forEach((item,index)=>{
				if(item.docstatus == 0){
					msg = "Tiene un cierre por validar. No podrá realizar ventas hasta que valide el siguiente documento\n"+
						`<a href='/app/cierre-tienda/${item.name}' target='_blank' style='color:red;'>Click aqui</a>`
					frappe.throw({
						message: __(msg),
						indicator: 'red',
						title: __('Error'),
				...
				if (item.fecha_cierre == hoy) {
					let msg = "Tiene un cierre de caja del día de hoy. No podrá realizar ventas hasta el día de mañana";
					frappe.throw({
						message: __(msg),
						indicator: 'red',
						title: __('Error'),
				...
				if (item.fecha_cierre == fechaReferencia && item.docstatus == 1) {
					existence_cierre_referencia = 1
				}

			})

			if(existence_cierre_referencia == 0){
				if(ultima_venta.length > 0){
					msg = "No puede generar ventas, debe de cerrar la caja del dia "+fechaReferencia+" para poder continuar"
					frappe.throw({
						message: __(msg),
						indicator: 'red',
						title: __('Error'),
	...
	async validate_closing_store(){

		//const get_closing = await frappe.db.get_list("CIERRE TIENDA",{
		//	"filters":[["fecha_cierre","=","2023-06-06"],["docstatus","=",1]],
		//	"limit":"None"
		//})
		//if(get_closing.length > 0){
		//	console.log("closing",get_closing)
		//	frappe.throw("No puede generar ventas despues de su cierre de shalom store")
		//	frappe.utils.play_sound("error");
		//	return false;
		//}

		// console.log(get_closing)
