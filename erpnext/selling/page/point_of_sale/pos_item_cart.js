erpnext.PointOfSale.ItemCart = class {
		...
		this.$component.append(
			`<div class="cart-container">
				<div class="abs-cart-container">
					<div class="cart-label">Carrito de Articulos</div>
					<div class="cart-header">
						<div class="name-header">Articulo</div>
						<div class="qty-header">Cantidad</div>
						<div class="rate-amount-header">Monto</div>
					</div>
	...
	make_no_items_placeholder() {
		...
		this.$cart_items_wrapper.html(
			`<div class="no-item-wrapper">No hay articulos en el carrito</div>`
		);
	}
	...
	make_cart_totals_section() {
		...
		this.$totals_section.append(
			`<div class="net-total-container">
				<div class="net-total-label" style="font-size: 15px;"><strong>Total</strong></div>
				<div class="net-total-value" style="font-size: 15px;"><strong>0.00</strong></div>
			</div>
			<div class="taxes-container"></div>
			<div class="checkout-btn">Verificar y Continuar</div>
			<div class="edit-cart-btn">Editar Carrito</div>`
		)
	...
	bind_events() {
		...
		this.$cart_items_wrapper.on('click', '.trash', function() {

			console.error('Mu')
			console.error('By: sebas')

			const $cart_item = $(this);
			const nameRow = $cart_item[0].parentElement.dataset.rowName;

			console.log( nameRow, 'nameRow' )
			frappe.model.clear_doc( 'POS Invoice Item', nameRow)
	...
	update_totals_section(frm) {
		let discountGeneral = parseFloat(localStorage.getItem("discountGeneral"));
		if (!frm) frm = this.events.get_frm();
		let products = ["24112404 - MINI PAQUETERIA","24112404-PAQUETERIA","24112404-PAQUETE M"];
		let users = ["77050071@shalomcontrol.com","48191841@shalomcontrol.com","tienda@shalom.com.pe","74451380@shalomcontrol.com"]
		let new_total = 0;
		if(frm.doc.items.length>0){
			frm.doc.items.forEach((item)=>{
				if(users.includes(frappe.user.name)) {
				if (item.item_code.includes("24112404")) {
					let discount = item.qty * discountGeneral
					new_total += (item.amount - discount)
				} else {
					new_total += item.amount
				}
			}else{
				const item_price = this.descuentoProductos(item)
				new_total += item_price.new_amount
				// if (item.item_code.includes("24112404")) {
				// 	if (item.qty >= 15) {
				// 		let discount = item.qty * 0.3
				// 		new_total += (item.amount - discount)
				// 	} else {
				// 		new_total += item.amount
				// 	}
		...
		this.render_net_total(new_total);
	...
	render_net_total(value) {
		...
		this.$totals_section.find('.net-total-container').html(
			`<div style="font-size:15px;"><strong>Total</strong></div><div><strong>${format_currency(value, currency)}</strong></div>`
		)

		this.$numpad_section.find('.numpad-net-total').html(
			`<div style="font-size:15px;"><strong>Total:</strong> <span><strong>${format_currency(value, currency)}</strong></span></div>`
		);
	}
	...
	render_grand_total(value) {
		...
		this.$totals_section.find('.grand-total-container').html(
			`<div>Total</div><div>${format_currency(value, currency)}</div>`
		)

		this.$numpad_section.find('.numpad-grand-total').html(
			`<div>Total: <span>${format_currency(value, currency)}</span></div>`
		);
	}
	...
	render_cart_item(item_data, $item_to_update) {
		...
		$item_to_update.html(
			...
			${get_rate_discount_html(this)}
			<div class="trash" data-id="${item_data.idx}" style="cursor:pointer;">
		...
		function get_rate_discount_html(_this) {
			let products = ["24112404 - MINI PAQUETERIA", "24112404-PAQUETERIA", "24112404-PAQUETE M"];
			let users = ["77050071@shalomcontrol.com", "48191841@shalomcontrol.com", "tienda@shalom.com.pe","74451380@shalomcontrol.com"]
			let new_amount = 0;
			let new_rate = 0;
			let discountGeneral = parseFloat(localStorage.getItem("discountGeneral"))
			if (users.includes(frappe.user.name)) {
				if (item_data.item_code.includes("24112404")) {
					let discount = item_data.qty * discountGeneral
					new_rate = item_data.rate - discountGeneral
					new_amount = (item_data.amount - discount)
				} else {
					new_rate = item_data.rate
					new_amount = item_data.amount
				}
			} else {
				const item_price = _this.descuentoProductos(item_data)
				new_rate = item_price.new_rate
				new_amount = item_price.new_amount

				// if (item_data.item_code.includes("24112404")) {
				// 	if (item_data.qty >= 15) {
				// 		let discount = item_data.qty * 0.3
				// 		new_rate = item_data.rate - 0.3
				// 		new_amount = (item_data.amount - discount)
				// 	} else {
				// 		new_rate = item_data.rate
				// 		new_amount = item_data.amount
				...
				return `
					<div class="item-qty-rate">
						...
						<div class="item-rate-amount">
							<div class="item-rate">${format_currency(new_amount, currency)}</div>
							<div class="item-amount">${format_currency(new_rate, currency)}</div>
						</div>
					</div>`
			} else {
				return `
					<div class="item-qty-rate">
						<div class="item-qty"><span>${item_data.qty || 0}</span></div>
						<div class="item-rate-amount">
							<div class="item-rate">${format_currency(new_rate, currency)}</div>
						</div>
					</div>`
			}
		}
	...
	get_rate_discount_html(item_data) {
		let products = ["24112404 - MINI PAQUETERIA", "24112404-PAQUETERIA", "24112404-PAQUETE M"];
		let users = ["77050071@shalomcontrol.com", "48191841@shalomcontrol.com", "tienda@shalom.com.pe","74451380@shalomcontrol.com"]
		let new_amount = 0;
		let new_rate = 0;
		...
		if (users.includes(frappe.user.name)) {
			...
			const item_price = this.descuentoProductos(item_data)
			new_rate = item_price.new_rate
			new_amount = item_price.new_amount
	...
	toggle_customer_info(show) {
		if (show) {
			...
			this.$customer_section.find('.customer-details').html(
				...
				<div class="transactions-label">Transacciones Recientes</div>`
			);
	...
	descuentoProductos(item_data){
		...
		let new_rate = item_data.rate
		let new_amount = item_data.amount
		for(const [item_val,data_desc] of Object.entries(descuentos)){
			if(item_data.item_code.includes(item_val) || item_data.item_name.includes(item_val)){
				const precios_desc = data_desc["precio"]? Object.entries(data_desc["precio"]): Object.entries(data_desc["porcentaje"])
				for(let [interval_desc,item_desc] of precios_desc){
					const interval = interval_desc.split("-")
					if(interval.length === 0) continue;
					const min_val = parseInt(interval[0])
					const max_val = interval.length > 1? parseInt(interval[1]): 9999999999
					if(item_data.qty >= min_val && item_data.qty < max_val){
						let new_discount = data_desc["precio"]? item_desc: item_data.rate*item_desc
						let discount = item_data.qty * new_discount
						new_rate = item_data.rate - new_discount
						new_amount = (item_data.amount - discount)
						break;
					}
				}
