...
def get_data():
	filters_dict = frappe.form_dict.company
	return filters_dict
	filters = {
		"company": filters_dict.get("company"),
		"to_date": filters_dict.get("to_date"),
		"warehouse": filters_dict.get("warehouse"),
		"range1": filters_dict.get("range1"),
		"range2": filters_dict.get("range2"),
		"range3": filters_dict.get("range3"),
		"show_warehouse_wise_stock": filters_dict.get("show_warehouse_wise_stock")
	}
	...
	for item, item_dict in iteritems(item_details):
		...
		range1, range2, range3, above_range3 = get_range_age_data(filters, fifo_queue, to_date)
...
def get_data_supervision():
	...
	filters = {
		...
		"range1": int(frappe.form_dict.range1),
		"range2": int(frappe.form_dict.range2),
		"range3": int(frappe.form_dict.range3),
		"show_warehouse_wise_stock": int(frappe.form_dict.show_warehouse_wise_stock)
	}
	...
	for item, item_dict in iteritems(item_details):
		...
		row.extend([item_dict.get("qty_after_transaction"), average_age,
					range1, range2, range3, above_range3,
					earliest_age, latest_age, details.stock_uom])
...
def get_range_age_data(filters, fifo_queue, to_date):
	range1 = range2 = range3 = above_range3 = 0.0
	for item in fifo_queue:
		age = date_diff(to_date, item[1])

		if age <= filters.get("range1"):
			range1 += flt(item[0])
		elif age <= filters.get("range2"):
			range2 += flt(item[0])
		elif age <= filters.get("range3"):
			range3 += flt(item[0])
		else:
			above_range3 += flt(item[0])
