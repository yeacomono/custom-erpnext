...
def execute(filters=None):
	...
	# 	frappe.throw(_("El almacén es obligatorio para generar el reporte"))
	# 	return False
...
def report_stock():
	is_reposting_item_valuation_in_progress()
	include_uom = frappe.form_dict.include_uom
	...
	columns = get_columns()
	items = get_items_new(filters)
	sl_entries = get_stock_ledger_entries(filters, items)
	item_details = get_item_details(items, sl_entries, include_uom)
	opening_row = get_opening_balance_new(filters, columns)
	precision = cint(frappe.db.get_single_value("System Settings", "float_precision"))
	data = []
	conversion_factors = []
	if opening_row:
...
@frappe.whitelist(allow_guest=True)
def report_stock_ledger(date_init=None,date_end=None):
	...
	conditions = []
	warehouse_condition = get_warehouse_condition("ALMACEN GENERAL-TIENDA - SE")
	if warehouse_condition:
		conditions.append(warehouse_condition)
	condition_new_warehouse = "and {}".format(" and ".join(conditions)) if conditions else ""
	return condition_new_warehouse
	sl_entries = frappe.db.sql("""
		...
		WHERE
			company = 'Shalom Empresarial'
				AND is_cancelled = 0 and item_code='BOLSA DE SEGURIDAD-PEQUEÑO' AND posting_date BETWEEN '"""+ date_init +"""' AND '"""+ date_end +"""' """+condition_new_warehouse+"""
...
def get_opening_balance_new(filters, columns):
	if not (filters.get("item_code") and filters.get("warehouse") and filters.get("from_date")):
		return
