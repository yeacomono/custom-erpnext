...
def get_columns(filters):
	...
	columns = [
		...
		{"label": _("Precio de venta estándar"), "fieldname": "standard_rate", "fieldtype": "Float", "width": 80},
		{"label": _("Tasa de cambio de última compra"), "fieldname": "last_purchase_rate", "fieldtype": "Float", "width": 80},
...
def get_stock_ledger_entries(filters, items):
	...
	return frappe.db.sql("""
		select
			...
			sle.item_code as name, sle.voucher_no, sle.stock_value, sle.batch_no, it.standard_rate, it.last_purchase_rate, it.min_stock, it.cantidad_por_producto_mínimo
		from
			`tabStock Ledger Entry` sle force index (posting_sort_index)
		LEFT JOIN tabItem it  on (sle.item_code = it.item_code)
...
def get_stock_ledger_entries2(filters, items):
	...
	return frappe.db.sql("""
		select
			sle.item_code, warehouse,wh.sucursal, sle.posting_date, sle.actual_qty, sle.valuation_rate,
			sle.company, sle.voucher_type, sle.qty_after_transaction, sle.stock_value_difference,
			sle.item_code as name, sle.voucher_no, sle.stock_value, sle.batch_no, it.standard_rate, it.last_purchase_rate, it.min_stock, it.cantidad_por_producto_mínimo
		from
			`tabStock Ledger Entry` sle force index (posting_sort_index)
		LEFT JOIN tabItem it  on (sle.item_code = it.item_code)
		LEFT JOIN tabWarehouse wh  on (sle.warehouse = wh.name)
		where sle.docstatus < 2 %s %s
		and is_cancelled = 0
		order by sle.posting_date, sle.posting_time, sle.creation, sle.actual_qty""" % #nosec
						 (item_conditions_sql, conditions), as_dict=1)
	...
	for d in sle:
		...
		if key not in iwb_map:
			iwb_map[key] = frappe._dict({
				...
				"last_purchase_rate": d.last_purchase_rate,
				"branch": d.sucursal,
			})
...
def get_item_details(items, sle, filters):
	...
	res = frappe.db.sql("""
		select
			item.name, item.item_name, item.description, item.item_group, item.brand, item.stock_uom, item.min_stock, item.cantidad_por_producto_mínimo %s
		from
			`tabItem` item
			%s
		where
...
def report_stock_balance(filters=None):
	if not filters: filters = {}
	...
	include_uom = filters.get("include_uom")
	items = get_items(filters)
	sle = get_stock_ledger_entries2(filters, items)
