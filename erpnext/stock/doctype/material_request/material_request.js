            ...
            if (flt(frm.doc.per_ordered, precision) < 100) {
                ...
                if (frm.doc.material_request_type === "Material Transfer") {
                    ...
                    frm.add_custom_button(__("Transfer Material"),
                        () => {
                            let productos_en_tabla = frm.doc.items || []
                            let encontro_film = false
                            if (productos_en_tabla.length) {
                                for (let producto of productos_en_tabla) {
                                    if (producto.item_code == "24141502-GRANDE 20") encontro_film = true
                                }
                            }
                            if (encontro_film) {
                                if (frm.doc.aprobacion_de_solicitud == "En Espera") {
                                    frappe.throw({
                                        title: 'Aviso',
                                        message: "No puede generar la transferencia hasta que la solicitud sea aprobada"
                                    })
                                } else if (frm.doc.aprobacion_de_solicitud == "Rechazado") {
                                    frappe.throw({
                                        title: 'Aviso',
                                        message: "No puede generar la transferencia porque fue rechazada"
                                    })
                                }else{
