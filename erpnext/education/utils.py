...
def get_enrollment_new(master, document, student):
	...
	return master, document, student
...
def evaluate_quiz_new(quiz_response, quiz_name, course, program, time_taken):
	import json
	...
	if student:
		enrollment = get_or_create_course_enrollment_new(course, program)
...
def get_or_create_course_enrollment_old(course, program):
	student = get_current_student()
	course_enrollment = get_enrollment("course", course, student.name)
	if not course_enrollment:
