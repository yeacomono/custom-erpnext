...
def get_conditions(filters):
	...
	if filters.get("company"): conditions += " and inv.company=%(company)s"
	if filters.get("customer"): conditions += " and inv.customer = %(customer)s"

	if filters.get("from_date"): conditions += " and inv.posting_date >= %(from_date)s"
	if filters.get("to_date"): conditions += " and inv.posting_date <= %(to_date)s"

	if filters.get("owner"): conditions += " and inv.owner = %(owner)s"
...
def get_invoices(filters, additional_query_columns):
	...
	return frappe.db.sql("""
		select inv.name, inv.posting_date as posting_date, inv.debit_to as debit_to, inv.project as project, inv.customer as customer,
		inv.customer_name as customer_name, us.mobile_no as owner, inv.remarks as remarks, inv.territory as territory, inv.tax_id as tax_id, inv.customer_group as customer_group,
		inv.base_net_total as base_net_total, inv.base_grand_total as base_grand_total, inv.base_rounded_total as base_rounded_total, inv.outstanding_amount as outstanding_amount,
		inv.is_internal_customer as is_internal_customer, inv.represents_company as represents_company, inv.company as company {0}
		from `tabSales Invoice` inv
		LEFT JOIN `tabUser` us on us.name = inv.owner
		where inv.docstatus = 1 %s order by inv.posting_date desc, inv.name desc""".format(additional_query_columns or '') %
