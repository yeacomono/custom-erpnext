...
def get_stock_availability(item_code, warehouse):
	...
	return bin_qty[0].actual_qty if bin_qty else 0

@frappe.whitelist(allow_guest=True)
def get_stock_availability_all(item_code, warehouse):

	values = {'names': item_code }
	...
	for item_data in data:
		item_group_stock[item_data.name] = item_data.is_stock_item

	response_bin_qty = get_bin_qty_all(item_code, warehouse)
	response_bundle_availability = get_bundle_availability_all(item_code, warehouse)

	response_pos_sales_qty = get_pos_reserved_qty_all(item_code, warehouse)

	result_group = {}
	for item_iter in item_code:
		if item_iter in item_group_stock:
			if item_iter in response_bin_qty:
				bin_qty = response_bin_qty[item_iter]
				pos_sales_qty = response_pos_sales_qty[item_iter]
			else:
				bin_qty = 0
				pos_sales_qty = 0

			result_group[item_iter] = bin_qty - pos_sales_qty
		else:
			if frappe.db.exists('Product Bundle', item_code):
				if item_iter in response_bundle_availability:
					result_group[item_iter] = response_bundle_availability[item_iter]
				else:
					result_group[item_iter] = 0
	return result_group
...
def get_bundle_availability_all(bundle_item_code, warehouse):
	product_bundle = frappe.db.sql("""
    select * from `tabProduct Bundle` WHERE name IN %(names)s
""", values={'names': bundle_item_code}, as_dict=1)

	product_bundle_group = {}
	for item_data in product_bundle:
		product_bundle_group[item_data.item_code] = item_data.qty

	bundle_bin_qty = 1000000
	item_bin_qty_group = get_bin_qty_all(bundle_item_code, warehouse)
	item_pos_reserved_qty_group = get_pos_reserved_qty_all(bundle_item_code, warehouse)

	bundle_bin_qty_group = {}
	for item_it in bundle_item_code:
		item_bin_qty = 0
		item_pos_reserved_qty = 0
		if item_it in item_bin_qty_group:
			item_bin_qty = item_bin_qty_group[item_it]

		if item_it in item_pos_reserved_qty_group:
			item_pos_reserved_qty = item_pos_reserved_qty_group[item_it]
		available_qty = item_bin_qty - item_pos_reserved_qty

		max_available_bundles = 0
		if item_it in product_bundle_group:
			max_available_bundles = available_qty / product_bundle_group[item_it]

		if bundle_bin_qty > max_available_bundles:
			bundle_bin_qty = max_available_bundles

		lit_bundle = bundle_bin_qty - item_pos_reserved_qty_group[item_it]
		bundle_bin_qty_group[item_it] = lit_bundle
	return bundle_bin_qty_group
...
def get_bin_qty_all(item_code, warehouse):
	bin_qty = frappe.db.sql("""select actual_qty,item_code from `tabBin`
		where item_code IN %s and warehouse = %s
		order by creation desc""", (item_code, warehouse), as_dict=1)

	item_group_bin = {}
	for item_data in bin_qty:
		if not item_data.item_code in item_group_bin:
			item_group_bin[item_data.item_code] = item_data.actual_qty

	for item_ite in item_code:
		if not item_ite in item_group_bin:
			item_group_bin[item_ite] = 0

	return item_group_bin

def get_pos_reserved_qty(item_code, warehouse):

	list = ["TIENDA-CANTA CALLAO SMP - SE","TIENDA-VMT - SE","TIENDA-PUEBLO LIBRE - SE","TIENDA-SURCO - SE"]

	if warehouse in list:
...
def get_pos_reserved_qty_all(item_code, warehouse):

	list = ["TIENDA-CANTA CALLAO SMP - SE","TIENDA-VMT - SE","TIENDA-PUEBLO LIBRE - SE","TIENDA-SURCO - SE"]
	list_pos_reserved = {}
	if warehouse in list:

		reserved_qty_group = frappe.db.sql("""select p_item.qty as qty,p_item.item_code
			from `tabPOS Invoice` p, `tabPOS Invoice Item` p_item
			where p.name = p_item.parent
			and ifnull(p.consolidated_invoice, '') = ''
			and p_item.docstatus = 1
			and p_item.item_code IN %s
			and p_item.warehouse = %s
			and p.posting_date > '2022-11-30'
		""", (item_code, warehouse), as_dict=1)

		for item_data in reserved_qty_group:
			if not item_data.item_code in list_pos_reserved:
				list_pos_reserved[item_data.item_code] = 0
			list_pos_reserved[item_data.item_code] += item_data.qty

		for item_ite in item_code:
			if not item_ite in list_pos_reserved:
				list_pos_reserved[item_ite] = 0

		return list_pos_reserved

	else:

		reserved_qty_group = frappe.db.sql("""select p_item.qty as qty,p_item.item_code as item_code
			from `tabPOS Invoice` p, `tabPOS Invoice Item` p_item
			where p.name = p_item.parent
			and ifnull(p.consolidated_invoice, '') = ''
			and p_item.docstatus = 1
