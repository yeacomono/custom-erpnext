	...
	refresh: function(frm) {
		if (!frm.doc.__islocal) {
			if (frm.doc.__onload && frm.doc.__onload.job_offer) {
				...
				frm.add_custom_button(__("Job Offer"), async function() {

					let search = await frappe.db.get_list("Job Offer",{
						filters:[["job_applicant","=",cur_frm.doc.name]],
						limit:"None"
					})
					...
					if(search.length > 0){
						...
						frappe.set_route("Form", "Job Offer", search.name);
