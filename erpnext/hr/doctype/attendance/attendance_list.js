frappe.listview_settings['Attendance'] = {
	...
	get_indicator: function (doc) {
		...
		} else if (doc.status == "DM Observado") {
			return [__(doc.status), "yellow", "status,=," + doc.status];
		} else if (doc.status == "Marcacion Incompleta") {
			console.log("wow")
			...
			let dialog = new frappe.ui.Dialog({
				...
				fields: [
					...
					},
					{
						...
						fieldtype: "Select",
						fieldname: "status",
						options: ["Present", "Absent", "Half Day", "Work From Home", "DM Observado","Marcacion Incompleta"],
						hidden:1,
						reqd: 1,
