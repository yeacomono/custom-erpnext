...
def get_employees(date, department = None, branch = None, company = None):
	...
	attendance_marked = []
	filters = {"status": "Active", "fecha_de_ingreso_real": ["<=", date]}
	...
	employee_list = frappe.get_list("Employee", fields=["employee", "nombre_completo","fecha_de_relevo"], filters=filters, order_by="nombre_completo")
	marked_employee = {}
	...
	for employee in employee_list:
		...
		if employee['employee'] not in marked_employee:
			if employee['fecha_de_relevo'] != None:
				if str(employee['fecha_de_relevo']) >= date:
