...
erpnext.EmployeeSelector = Class.extend({
	...
	make: async function(frm, employee) {
		var me = this;
		$(this.wrapper).empty();
		...
		// const employee_data_2 = await frappe.db.get_list('Employee',{
		// 	'fields':['fecha_de_ingreso_real','name'],
		// 	'limit':'None'
		// });
		var row;
		$.each(employee, function(i, m) {
			// if(searchDateIngress(m.employee,employee_data_2,frm.doc.date)){
			if (i===0 || (i % 4) === 0) {
				row = $('<div class="row"></div>').appendTo(me.wrapper);
			}
			...
			%(employee)s</label>\
			</div></div>', {employee: m.nombre_completo})).appendTo(row);
			// }
		});
...
function searchDateIngress(id,data,fecha){
	let valor = false
	data.forEach((item,index)=>{
		if(item.name == id && fecha>=item.fecha_de_ingreso_real){
		valor = true;
	}
})
	return valor;
