			...
			if (data.length) {
				...
				me.page.main.find('.patient_documents_list').append(`
					<div class='text-muted' align='center'>
						<br><br>${__('No hay más registros..')}<br><br>
					</div>`);
...
let add_to_records = function(me, data) {
	...
	for (i=0; i<data.length; i++) {
		if (data[i].reference_doctype) {
			...
			let dateParsed = data[i].date_sep  != ""? moment(data[i].communication_date).format("D MMMM YYYY").toString().toLocaleUpperCase() : ""
			let referenceDoctype = "FICHA OCUPACIONAL"
			let label = '';
			if (data[i].subject) {
			...
			time_line_heading += referenceDoctype + " - " +
